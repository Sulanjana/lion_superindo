...
const DeleteProductVariant = ({variant}: {variant: productVariant}) => {
  const router = useRouter();
  const [openModal, setOpenModal] = useState(false);

  const handleDelete = async (productVariant: number) => {
    await axios.delete(`/api/productVariant/${productVariant}`);
    router.refresh();
    setOpenModal(false);
  };
  return (
